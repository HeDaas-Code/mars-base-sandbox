...
# ============================================================
# 模拟游戏状态（Phase 1 硬编码，Phase 2 接入 GameState）
# ============================================================
...
def _process_natural_language(
    ...
    # 游戏状态字符串（Phase 1 硬编码）
    game_state = f"Sol {MOCK_GAME_STATE['sol']}, 信号质量 {MOCK_GAME_STATE['signal_quality_pct']}%"
    ...
    # 公式: 500 + (100 - signal_quality_pct) * 50
    latency_ms = 500 + (100 - MOCK_GAME_STATE["signal_quality_pct"]) * 50

    # 构造 agent_message
    response_text = result["response"]

    # 情绪提示（Phase 1 基于关键词匹配）
    ...
    msg = build_agent_message(
        ...
        latency_ms=latency_ms,
        emotion_hint=emotion_hint,
        signal_quality_pct=MOCK_GAME_STATE["signal_quality_pct"],
    )
