...
def init_seed_memories():
    """初始化全部6个 NPC 的种子记忆到 ChromaDB

    在 WebSocket 服务器启动时调用一次。
    已有记忆的 NPC 会被跳过。
    """
    ...
    all_memories = get_all_seed_memories()
    total_loaded = 0

    for npc_id, memories in all_memories.items():
        existing = store.count(npc_id)
        if existing > 0:
            logger.info("%s: 已有 %d 条记忆，跳过", npc_id, existing)
            continue
        store.batch_add(memories)
        total_loaded += len(memories)
        logger.info("%s: 加载 %d 条种子记忆", npc_id, len(memories))

    logger.info("种子记忆初始化完成: 共 %d 条", total_loaded)
