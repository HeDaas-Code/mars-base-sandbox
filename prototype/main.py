...
def init():
    ...
    for npc_id, memories in all_memories.items():
        ...
        if existing > 0:
            print(f"[INFO] {npc_id}: 已有 {existing} 条记忆，跳过")
            continue
        for mem in memories:
            store.add_memory(
                ...
                tags=mem["tags"].split(",") if mem.get("tags") else [],
            )
        total_loaded += len(memories)
        print(f"[INFO] {npc_id}: 加载 {len(memories)} 条种子记忆")
    print(f"[INFO] 共加载 {total_loaded} 条记忆\n")
